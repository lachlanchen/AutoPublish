...
- `python load_env.py`: loads the repo-local `.env` first, then falls back to `~/.bashrc` only for missing values; use `.env` as the default source of truth for secrets.
...
- Keep real secrets out of git—copy `.env.example` to `.env` and populate `SENDGRID_API_KEY`, `APP_PASSWORD`, `APIKEY_2CAPTCHA`, etc.; `.env` is ignored and should be the primary runtime secret source.
